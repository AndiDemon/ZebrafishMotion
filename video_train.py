...
def main():
    ...
    # vec, label = videoDataset.load_data(data_root + "o1.mp4", 1)
    ...
    # ])))
    #
    source_o = videoDataset.load_data(data_root + "o1.mp4")
    source_y = videoDataset.load_data(data_root + "y1.mp4")
    train_loader = torch.utils.data.DataLoader(GenerateData(((source_y, 0), (source_o, 1))), batch_size=32, shuffle=False)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        print(batch_idx)
        print(data.shape)
        print(target.shape)# whatever you want
            # break
